...
MASTER_CAPSULES_DIR = os.path.join(TERROIR_ROOT, "SYSTEM", "MEMORIA", "capsulas_maestras")
RAW_LOGS_DIR = os.path.join(TERROIR_ROOT, "SYSTEM", "MEMORIA", "logs_de_sesion")
...
def clean_ai_json(text: str) -> str:
    ...
    elif "```" in text: text = re.search(r"```\s*(.*?)\s*```", text, re.DOTALL).group(1)
